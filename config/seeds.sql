INSERT INTO department(department_name)
VALUES ("Develop"),
       ("Engineering"),
       ("Administrate"),
       ("Data");

INSERT INTO roles(title, salary, department_id)
VALUES ("Software-Engineer", 150000, 1),
       ("Full-Stack-Developer", 100000, 4),
       ("Quality-Manager", 75000, 2),
       ("Lead-Developer", 175000, 4),
       ("Database-administrator", 85000, 2),
       ("Computer-systems-analyst", 90000, 3),
       ('Computer-systems-engineer', 110000, 1);

INSERT INTO employee(first_name, last_name, title)
VALUES ("Adrienne", "Cordell", "Full-Stack-Developer"),
       ("Owen", "Campbell", "Quality-Manager"),
       ("Jayden", "Benedict", "Database-administrator"),
       ("Elizabeth", "Ketola", "Computer-systems-analyst"),
       ("Alayna", "Witsman", "Lead-Developer"),
       ("Jared", "white", "Computer-systems-engineer");


